...
The system SHALL provide a frontend API client using `axios` with centralized configuration to create and delete anonymous sessions via `/api/v1/sessions` and `/api/v1/sessions/{session_id}`, returning page-consumable payloads aligned with OpenAPI schemas.
